    ...
    <div v-else class="events-list">
      <div v-for="event in events" :key="event.id" class="event-card">
        <div class="event-details">
          <h3 class="event-name">{{ event.nom || event.name }}</h3> <!-- Adjusted to common property names -->
          <p class="event-description">{{ truncateText(event.description, 100) }}</p>
          <div class="event-info">
            <!-- Assuming your Event entity has 'date' and 'time' fields -->
            <span><i class="bi bi-calendar-event"></i> {{ formatDate(event.date || event.eventDate) }}</span>
            <span><i class="bi bi-clock"></i> {{ formatTime(event.time || event.eventTime) }}</span>
          </div>
          <div class="event-info">
            <!-- Assuming your Event entity has 'lieu' or 'localisation' or 'location' -->
            <span><i class="bi bi-geo-alt-fill"></i> {{ event.lieu || event.localisation || event.location }}</span>
            <span :class="['event-status', getStatusClass(event.etat)]">{{ formatStatus(event.etat) }}</span>
          </div>
          <div class="event-actions">
            <router-link :to="`/events/details/${event.id}`" class="btn btn-sm btn-outline-primary">View Details</router-link>
            <button @click="navigateToUpdate(event.id)" class="btn btn-sm btn-outline-secondary">Edit</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchMyEvents() {
      ...
      try {
        ...
        if (err.response && err.response.status === 401) {
            this.error = 'You are not authorized to view these events. Please log in as a company.';
        } else if (err.response && err.response.status === 404) {
            this.error = 'No company profile found for your account. Please contact support.';
        }
        else {
            this.error = 'Failed to load your company events. Please try again later.';
        }
    ...
    formatStatus(status) {
        ...
        return status.replace('_', ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase()); // e.g., EN_ATTENTE -> En Attente
    },
...
.status-en-attente { background-color: #ffc107; color: #333; } /* EN_ATTENTE */
.status-accepte { background-color: #28a745; color: white; }    /* ACCEPTE */
.status-rejete { background-color: #dc3545; color: white; }     /* REJETE */
.status-termine { background-color: #17a2b8; color: white; }    /* TERMINE */
.status-annule { background-color: #6c757d; color: white; }     /* ANNULE */
